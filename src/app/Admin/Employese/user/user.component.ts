...
export class UserComponent extends BaseComponent implements OnInit {
  ...
  loadPage(page) { 
    this._api.post('/api/users/search',{page: page, pageSize: this.pageSize}).subscribe(res => {
      this.users = res.data;
  ...
  search() { 
    this.page = 1;
    this.pageSize = 5;
    this._api.post('/api/users/search',{page: this.page, pageSize: this.pageSize, hoten: this.formsearch.get('hoten').value, taikhoan: this.formsearch.get('taikhoan').value}).subscribe(res => {
      this.users = res.data;
      this.totalRecords =  res.totalItems;
      this.pageSize = res.pageSize;
      });
  ...
  onSubmit(value) {
    ...
    if(this.isCreate) { 
      this.getEncodeFromImage(this.file_image).subscribe((data: any): void => {
        ...
        this._api.post('/api/users/create-user',tmp).subscribe(res => {
      ...
      this.getEncodeFromImage(this.file_image).subscribe((data: any): void => {
        ...
        this._api.post('/api/users/update-user',tmp).subscribe(res => {
  ...
  public openUpdateModal(row) {
    ...
    setTimeout(() => {
      $('#createUserModal').modal('toggle');
      this._api.get('/api/users/get-by-id/'+ row.user_id).subscribe((res:any) => {
        this.user = res; 
